...
def build_task_control_graph() -> list[AgentGraphNode]:
    ...
    return [
        ...
        _node(
            ...
            next_on_success="read_onec_tasks",
            next_on_error="read_onec_tasks",
        ),
        _node(
            "read_onec_tasks",
            AgentGraphNodeType.TOOL_CALL,
            "Прочитать задачи из 1С",
            "Ищет задачи и поручения в 1С в read-only режиме.",
        ...
        _node(
            ...
            next_on_success="analyze_collected_tasks",
            next_on_error="final_failed",
        ),
        _node(
            "analyze_collected_tasks",
            AgentGraphNodeType.TOOL_CALL,
            "Проанализировать собранные поручения",
            "Анализирует поручения из Outlook и 1С.",
            tool_name="llm.analyze_collected_data",
            next_on_success="analyze_confidence",
            next_on_error="analyze_confidence",
        ),
        _node(
            "analyze_confidence",
...
def build_meeting_agent_graph() -> list[AgentGraphNode]:
    ...
    return [
        ...
        _node(
            ...
            tool_name="outlook.read_calendar",
            next_on_success="analyze_calendar_load",
            next_on_error="final_failed",
        ),
        _node(
            "analyze_calendar_load",
            AgentGraphNodeType.TOOL_CALL,
            "Проанализировать нагрузку календаря",
            "Оценивает плотность совещаний, перегрузки, свободные окна и риски.",
            tool_name="llm.analyze_collected_data",
            next_on_success="build_schedule_recommendations",
            next_on_error="final_failed",
        ),
        _node(
            "build_schedule_recommendations",
            AgentGraphNodeType.TOOL_CALL,
            "Сформировать рекомендации по графику",
            "Формирует рекомендации по планированию дня или недели на основе календаря и аналитики.",
...
def build_document_analysis_graph() -> list[AgentGraphNode]:
    ...
    return [
        ...
        _node(
            ...
            next_on_success="search_onec_documents",
            next_on_error="search_onec_documents",
        ),
        _node(
            "search_onec_documents",
            AgentGraphNodeType.TOOL_CALL,
            "Найти документы в 1С",
            "Ищет связанные документы в 1С в read-only режиме.",
            tool_name="onec.search_documents",
            next_on_success="search_web_sources",
            next_on_error="search_web_sources",
        ),
        _node(
            "search_web_sources",
            AgentGraphNodeType.TOOL_CALL,
            "Найти внешнюю информацию",
            "Ищет дополнительную информацию через браузер или поисковый источник.",
        ...
        _node(
            ...
            next_on_success="llm_analyze_document",
            next_on_error="final_failed",
        ),
        _node(
            "llm_analyze_document",
            AgentGraphNodeType.TOOL_CALL,
            "Выполнить аналитический вывод",
            "Анализирует данные из документа, 1С, базы знаний и web-источников.",
            tool_name="llm.analyze_collected_data",
            next_on_success="build_report",
            next_on_error="build_report",
        ),
