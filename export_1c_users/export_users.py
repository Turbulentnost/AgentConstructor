...
def load_config(env_path: Path | None = None) -> dict[str, str]:
    ...
    return {
        ...
        "output": os.getenv("OUTPUT_PATH", "Instruct/exports/erp_pm_users_export.txt"),
    }
...
def load_users(conn: pyodbc.Connection, table: str) -> list[UserRow]:
    # CONVERT style 2 = hex without 0x
    sql = f"""
        SELECT
            ...
            CONVERT(varchar(max), Data, 2) AS PasswordDataHex
        FROM {table}
        ORDER BY Name
    """
...
def write_tsv(path: Path, users: list[UserRow], email_source: str, notes: list[str]) -> None:
    ...
    with path.open("w", encoding="utf-8-sig", newline="\n") as f:
        ...
        for note in notes:
            f.write(f"# {note}\n")
        f.write("Login\tDisplayName\tEmail\tOSName\tUserIdHex\tPasswordDataHex\n")
        for u in users:
