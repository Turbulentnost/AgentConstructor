...
Алгоритм: llm_proxy_service.onec.password
Этот CLI — thin-wrapper для локальной проверки.
"""
...
from llm_proxy_service.onec.password import verify_password_details  # noqa: E402
...
def load_user_data_from_export(export_path: Path, user_substr: str) -> tuple[str, bytes]:
    for line in export_path.read_text(encoding="utf-8-sig").splitlines():
        ...
        if len(parts) < 6:
            continue
        return parts[0], bytes.fromhex(parts[5])
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.sql:
        ...
        export = args.export or Path(__file__).resolve().parents[1] / "Instruct/exports/erp_pm_users_export.txt"
